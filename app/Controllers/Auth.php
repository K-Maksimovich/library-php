...
class Auth
{
    ...
    {
        header('Location: /backend/library/register');
        exit();
    }
    public static function exLog()
    {
        header('Location: /backend/library/login');
        exit();
    }
    ...
    public function register($data)
    {
        ...
        if (strlen($full_name) <= 5){
            ...
            $_SESSION['message-g'] = 'Вы успешно Зарегестрированны';
            header('Location: /backend/library/');
            exit();
        }
    ...
    public function login($data)
    {
        ...
        if (mysqli_num_rows($check_user) > 0){
            ...
            $_SESSION['user'] = [
                ...
                "email" => $user['email'],
                "avatar" => "",
                "user_gr" => "" /* Значение от 1 до 3, Пользователь, Автор, Админ */
            ];
...
//        $this->is_avatar();
    ...
    public function imgprofile($data, $files)
    {
        ...
        if (move_uploaded_file($avatar["tmp_name"], $path)){
            mysqli_query(App::$connect, "UPDATE `users` SET `img_profile` =
                '$sql_path_avatar' WHERE `users`.`id` = '$id'");
        } else{
            Router::error(500);
        }
        $check_avatar = mysqli_query(App::$connect, "SELECT * FROM `users` WHERE `img_profile` = '$sql_path_avatar'");
        $avatars_path_forIMG = mysqli_query(App::$connect, "SELECT `img_profile` FROM `users`");
        ...
        if (mysqli_num_rows($check_avatar) > 0){
            ...
            header('Location: /backend/library/profile');
            exit();
        } else{
    ...
    public static function is_avatar()
    {
        $id = $_SESSION['user']['id'];
//
        $check_avatar = mysqli_query(App::$connect, "SELECT `img_profile` FROM `users` WHERE `id` = '$id'");
...
//        $who_group = mysqli_query(App::$connect, "SELECT `group_user` FROM `users`.`id` = '$id'");
